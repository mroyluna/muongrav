function forward_calculation_geo(n, height, X, Y, Elev, enable_plotting)
...
%Constants
rock_density=2200;
delta_rock_density=2100 - rock_density;
delta_rock_density2=1420-2100;
...
Layer=(YI-min(min(YI))).*LayerSlopeN + (XI-min(min(XI))).*LayerSlopeE + LayerElev;
Layer2=height.*ones(n);
...
plot (YI(:,1),minElev2((eastingcut*n)-(n-1):eastingcut*n)','o')
...
filename=['AllData_', num2str(n),'_',num2str(rock_density),'_',num2str(delta_rock_density),'_',num2str(delta_rock_density2),'_',num2str(LayerElev),'_', num2str(height),'.dat'];

...
methods (Static)
    function write_from_txt(file_name_no_ext)
        ...
        data = textscan(fid, '%d %d %f %f %f', 'HeaderLines', 1, 'Delimiter', ',');
        %number of %d above is 2 for latest file from Kelley -- 11/16/16
        fclose(fid);

        binary_file = fopen([file_name_no_ext '.bin'], 'w');
        fwrite(binary_file, horzcat(data{3:5}), 'single');
        fclose(binary_file);
    end

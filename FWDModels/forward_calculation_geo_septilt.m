function forward_calculation_geo_septilt(n, LayerSlopeNU, LayerSlopeEU, LayerSlopeNL, LayerSlopeEL, XI, YI, ElevI, X, Y, Elev, Layer, Layer2, delta_rock_density, delta_rock_density2, dirname, enable_plotting)
%n, LayerSlopeN, LayerSlopeE,
...
freeairerr=.3086*2*ElevErr;%from grav guide
...
dataarray=[easting',northing',elevations',offset_gz_vals,gz_avg_at_stations,gz_error_at_stations, LayerSlopeNU*ones(size(elevations')),LayerSlopeEU*ones(size(elevations')), LayerSlopeNL*ones(size(elevations')),LayerSlopeEL*ones(size(elevations'))];

eval(['cd ' dirname])           
filename=['AllDataSepTilt_', num2str(n),'_', num2str(LayerSlopeNU),'_', num2str(LayerSlopeEU),num2str(LayerSlopeNL),'_', num2str(LayerSlopeEL),'.dat'];
